...
const TodoList = () => {
  ...
  const [isTogglee, setIsTogglee] = useState(true)
    ...
    <div>
      {todos.map((todo) => (
        <Board key={todo.id} id={todo.id} >
          ...
          {isTogglee && <AddTodo onClick={()=> setIsTogglee(false)}><div>+ Add another list</div></AddTodo>}
          {!isTogglee && <TodoBoard setIsToggle={setIsTogglee}/>}
        </Board>
      ))}
    </div>
  );
