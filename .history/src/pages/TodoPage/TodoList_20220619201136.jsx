...
import { showList } from '../../store/slices/todoSlice'
import { TodoBoard } from "./TodoBoard";

const TaskList = () => {
  const todos = useSelector((state) => state.todo.todos);
  const addBoard = useSelector(state => state.todo.isShowList)
    ...
    <div>
      {todos.map((todo) => (
        <Board key={todo.id} id={todo.id} >
          ...
          {addBoard && <TodoBoard/> || <AddTodo onClick={addBoardHandler}><div>+ Add another list</div></AddTodo>}
        </Board>
      ))}
    </div>
  );
